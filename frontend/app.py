...
class ApplicationGUI(ttkbootstrap.Frame):
    def __init__(self, master, **kwargs):
        ...
        self.frame = ttkbootstrap.Labelframe(self, text="Einstellungen:", padding=15)
        self.frame.pack(fill=tk.X, expand=tk.YES)#grid(row=0, column=0, columnspan=3, sticky="ew")

        # URL-Input
        self.url = ""
        self.browser = tk.StringVar()
        self.frame_url = ttkbootstrap.Frame(self.frame)
        self.frame_url.pack(fill=tk.X, expand=tk.YES)#grid(row=0, column=0, columnspan=3, sticky="ew")
        self.label_url = ttkbootstrap.Label(self.frame_url, text="URL:", width=12)
        self.label_url.pack(side="left", padx=(15,0))
        self.entry_url = ttkbootstrap.Entry(self.frame_url)
        self.entry_url.pack(side="left", fill=tk.X, expand=tk.YES, padx=5)#grid(row=0, column=1)
        self.dropdown_url = ttkbootstrap.Combobox(self.frame_url, textvariable=self.browser, width=12)
        ...
        self.button_url = ttkbootstrap.Button(self.frame_url, text="Laden!", width=8, command=self.load_url)
        self.button_url.pack(side="left", padx=5)#grid(row=1, column=2, sticky='nsew')

        # Excel-Input
        self.file_path_excel = ""
        self.frame_excel = ttkbootstrap.Frame(self.frame)
        self.frame_excel.pack(fill=tk.X, expand=tk.YES, pady=15)#.grid(row=1, column=0)
        self.label_excel = ttkbootstrap.Label(self.frame_excel, text="Excel-Datei:", width=12)
        self.label_excel.pack(side="left", padx=(15,0))#.grid(row=3, column=0, sticky="w")
        self.entry_excel = ttkbootstrap.Entry(self.frame_excel, text=self.file_path_excel)
        self.entry_excel.pack(side="left", fill=tk.X, expand=tk.YES, padx=5)#.grid(row=3, column=1)
        self.button_excel_search = ttkbootstrap.Button(self.frame_excel, text="Durchsuchen...", bootstyle="secondary",width=12, command=self.search_file_path)
        self.button_excel_search.pack(side="left", padx=5)#.grid(row=3, column=2, sticky='nsew')
        self.button_excel_load = ttkbootstrap.Button(self.frame_excel, text="Laden!", width=8, command=self.load_file_path)
        self.button_excel_load.pack(side="left", padx=5)#.grid(row=4, column=2, sticky='nsew')
        
        # Misc settings
        self.one_shot_var = tk.BooleanVar()
        self.frame_misc = ttkbootstrap.Frame(self.frame)
        self.frame_misc.pack(fill=tk.X, expand=tk.YES, pady=15)#.grid(row=1, column=0)
        self.check_oneshot = ttkbootstrap.Checkbutton(self.frame_misc, text="Oneshot", style="Squaretoggle.Toolbutton", variable=self.one_shot_var)
        self.check_oneshot.pack(side="right")
        
        # Start?
        self.sep = ttkbootstrap.Separator(self.frame, orient=tk.HORIZONTAL)
        self.sep.pack(fill=tk.X, expand=tk.YES)
        self.frame_process = ttkbootstrap.Frame(self.frame)
        self.frame_process.pack(fill=tk.X, expand=tk.YES, pady=15)#.grid(row=2, column=0)
        self.button_start = ttkbootstrap.Button(self.frame_process, text="Start!", bootstyle="success", width=8, command=self.process)
        self.button_start.pack(side="right", padx=5)#grid(row=7, column=2, sticky="nsew")
        #self.button_start.pack()

        # Reset?
        self.button_reset = ttkbootstrap.Button(self.frame_process, text="Reset!", bootstyle="secondary", width=8, command=self.reset)
        self.button_reset.pack(side="right", padx=5)#grid(row=7, column=0, sticky="nsew")

        self.table_frame = ttkbootstrap.Frame(self)
